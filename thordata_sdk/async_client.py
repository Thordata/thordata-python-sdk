...
class AsyncThordataClient:
    ...
    def __init__(self, scraper_token: str, public_token: str, public_key: str, 
        ...
        Args:
            scraper_token (str): Token for Creating Tasks & SERP.
            public_token (str): Token for Status/Download.
            public_key (str): Key for Status/Download.
    ...
    async def get(self, url: str, **kwargs) -> aiohttp.ClientResponse:
        ...
        User is responsible for calling await response.text() or .json().
        """
    ...
    async def serp_search(self, query: str, engine: str = "google", num: int = 10, **kwargs) -> Dict[str, Any]:
        """Async SERP search."""
        ...
        async with self._session.post(self.SERP_API_URL, data=payload, headers=headers) as response:
    ...
    async def create_scraper_task(self, file_name: str, spider_id: str, individual_params: Dict[str, Any], 
        ...
        async with self._session.post(self.SCRAPER_BUILDER_URL, data=payload, headers=headers) as response:
            response.raise_for_status()
            data = await response.json()
    ...
    async def get_task_status(self, task_id: str) -> str:
        ...
        async with self._session.post(self.SCRAPER_STATUS_URL, data=payload, headers=headers) as response:
            data = await response.json()
            if data.get("code") == 200 and data.get("data"):
                for item in data["data"]:
                    if str(item["task_id"]) == str(task_id): return item["status"]
            return "Unknown"
    ...
    async def get_task_result(self, task_id: str, file_type: str = "json") -> str:
        ...
        async with self._session.post(self.SCRAPER_DOWNLOAD_URL, data=payload, headers=headers) as response:
            data = await response.json()
            if data.get("code") == 200: return data["data"]["download"]
